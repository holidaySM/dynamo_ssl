...
class Trainer:
    ...
    def _init_dino_head(self):
        if self.dino_head is None:
            self.dino_head = hydra.utils.instantiate(self.cfg.ssl.dino_head)
            if self.cfg.sync_bn:
                self.dino_head = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    self.dino_head
                )
            self.dino_head_optim = torch.optim.AdamW(
                params=self.dino_head.parameters(),
                lr=self.cfg.ssl_lr,
                weight_decay=self.cfg.ssl_weight_decay,
                betas=tuple(self.cfg.betas))
            self.dino_head, self.dino_head_optim = self.accelerator.prepare(self.dino_head, self.dino_head_optim)
    ...
    def train(self):
        ...
        for data in pbar:
            ...
            if self.cfg.clip_grad_norm:
                ...
                self.accelerator.clip_grad_norm_(
                    self.dino_head.parameters(), self.cfg.clip_grad_norm
                )
                self.accelerator.clip_grad_norm_(
            ...
            self.dino_head_optim.step()
            self.projector_optim.step()
            self.ssl.step()

            self.encoder_optim.zero_grad(set_to_none=True)
            self.dino_head_optim.zero_grad(set_to_none=True)
    ...
    def adjust_lr(self):
        ...
        # fmt: on
        optimizers = [self.encoder_optim, self.projector_optim, self.dino_head_optim]
